...
import markdown
from .models import MyUser,Processes,ProcessComments,Position
...
class ProcessDetailView(LoginRequiredMixin,FormMixin,DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.position.position_name != 'Boss':
            next_users= MyUser.objects.filter(position=Position.objects.get(pk=self.request.user.position.id+1)).all()
...
class AccountManageView(LoginRequiredMixin,ListView):
    '''管理帐号,显示账号基本概况.有两种情况,如果当前用户是管理员权限,则显示系统所有的用户信息;否则只显示当前用户信息'''
    login_url = '/accounts/login/'
    redirect_field_name = 'redirect_to'
